...
function submitUpdate(item) {
    ...
    state.patch(route('camp-budgets.update', item.id), { preserveScroll: true });
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-4">
            ...
            >
                <h3 class="text-base font-semibold text-app-ink dark:text-app-ink-dark">Nieuwe kampbegroting</h3>
                <div class="grid gap-3 sm:grid-cols-2">
                    ...
                    <textarea v-model="form.content" rows="8" class="rounded border border-app-border px-3 py-2 sm:col-span-2" placeholder="Werk hier de begroting uit..." />
                </div>
            ...
            <div class="surface-brand-top space-y-3 rounded-xl border border-app-border bg-app-panel p-4 shadow-sm dark:border-brand-blue/30 dark:bg-app-panel-dark">
                ...
                <div v-for="item in props.items" :key="`budget-${item.id}`" class="rounded-lg border border-app-border bg-white p-3">
                    <form class="grid gap-3 sm:grid-cols-2" @submit.prevent="submitUpdate(item)">
                        ...
                        <textarea v-model="editState(item).content" rows="8" class="rounded border border-app-border px-3 py-2 sm:col-span-2" :disabled="!canUpdate" />
